...
import com.asymmetrix.grc.common.utils.KRIUtils;
...
public class AuditingJobAspect {
	...
	private void saveAuditLog(String action, String message, String actionResult) {
		String userName = KRIConstants.ANONYMOUS_USER;
	...
	private void saveAccessDeniedLog(JoinPoint jp) {
		...
		if (ANONYMOUS_USER.equals(event.getAuthentication().getName())
			...
			throw new KRIException(ERROR_AUTHORIZATION_HEADER_NOT_FOUND);
		}
		auditRepo.save(
				getAuditLogObj(getUserName(event), KRIUtils.toUpperWithSpeChar(getMethodName(event), SPEC_CHAR_HYPHEN),
						PRE_AUTHORIZE, getIpAddress(), getErrorMsg(event)));
	}
