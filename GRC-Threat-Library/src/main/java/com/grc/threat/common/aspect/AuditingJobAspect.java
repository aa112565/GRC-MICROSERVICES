package com.grc.threat.common.aspect;
...
import com.grc.threat.common.aspect.repository.AuditLogRepository;
import com.grc.threat.common.config.authService.IAuthenticationDetails;
import com.grc.threat.common.constants.ThreatLibraryConstants;
import com.grc.threat.common.exception.ThreatLibraryException;
import com.grc.threat.common.utils.ThreatLibraryUtils;
...
public class AuditingJobAspect {
	...
	private static final String PRE_AUTHORIZE_LISTENER = "execution(* com.grc.threat.common.config.listener.CustomAuthorizationAuditListener.*(..))";
	...
	private void saveAuditLog(String action, String message, String actionResult) {
		String userName = ThreatLibraryConstants.ANONYMOUS_USER;
	...
	private void saveAccessDeniedLog(JoinPoint jp) {
		...
		if (ANONYMOUS_USER.equals(event.getAuthentication().getName())
			...
			throw new ThreatLibraryException(ERROR_AUTHORIZATION_HEADER_NOT_FOUND);
		}
		auditRepo.save(getAuditLogObj(getUserName(event),
				ThreatLibraryUtils.toUpperWithSpeChar(getMethodName(event), SPEC_CHAR_HYPHEN), PRE_AUTHORIZE,
