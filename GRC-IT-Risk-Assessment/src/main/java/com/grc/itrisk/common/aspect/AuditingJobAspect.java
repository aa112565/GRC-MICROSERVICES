package com.grc.itrisk.common.aspect;
...
import com.grc.itrisk.common.aspect.repository.AuditLogRepository;
import com.grc.itrisk.common.config.authService.IAuthenticationDetails;
...
public class AuditingJobAspect {
	...
	private static final String PRE_AUTHORIZE_LISTENER = "execution(* com.grc.itrisk.common.config.listener.CustomAuthorizationAuditListener.*(..))";
	...
	private void saveAuditLog(String action, String message, String actionResult) {
		String userName = ITRiskConstants.ANONYMOUS_USER;
	...
	private void saveAccessDeniedLog(JoinPoint jp) {
		...
		if (ANONYMOUS_USER.equals(event.getAuthentication().getName())
			...
			throw new ITRiskException(ERROR_AUTHORIZATION_HEADER_NOT_FOUND);
		}
		auditRepo.save(getAuditLogObj(getUserName(event),
				ITRiskUtils.toUpperWithSpeChar(getMethodName(event), SPEC_CHAR_HYPHEN), PRE_AUTHORIZE, getIpAddress(),
				getErrorMsg(event)));
	}
