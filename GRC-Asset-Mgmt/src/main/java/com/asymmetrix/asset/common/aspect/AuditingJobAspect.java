package com.asymmetrix.asset.common.aspect;
...
import com.asymmetrix.asset.common.config.authService.IAuthenticationDetails;
...
public class AuditingJobAspect {
	...
	private static final String PRE_AUTHORIZE_LISTENER = "execution(* com.asymmetrix.asset.common.config.listener.CustomAuthorizationAuditListener.*(..))";
	...
	private void saveAuditLog(String action, String message, String actionResult) {
		String userName = AssetConstants.ANONYMOUS_USER;
	...
	private void saveAccessDeniedLog(JoinPoint jp) {
		...
		if (ANONYMOUS_USER.equals(event.getAuthentication().getName())
			...
			throw new AssetException(ERROR_AUTHORIZATION_HEADER_NOT_FOUND);
		}
		auditRepo.save(getAuditLogObj(getUserName(event),
				AssetUtils.toUpperWithSpeChar(getMethodName(event), SPEC_CHAR_HYPHEN), PRE_AUTHORIZE, getIpAddress(),
				getErrorMsg(event)));
	}
