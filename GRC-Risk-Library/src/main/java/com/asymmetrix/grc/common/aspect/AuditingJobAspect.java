...
import com.asymmetrix.grc.common.utils.RiskUtils;
...
public class AuditingJobAspect {
	...
	private void saveAuditLog(String action, String message, String actionResult) {
		String userName = RiskConstants.ANONYMOUS_USER;
	...
	private void saveAccessDeniedLog(JoinPoint jp) {
		...
		if (ANONYMOUS_USER.equals(event.getAuthentication().getName())
			...
			throw new RiskException(ERROR_AUTHORIZATION_HEADER_NOT_FOUND);
		}
		auditRepo.save(
				getAuditLogObj(getUserName(event), RiskUtils.toUpperWithSpeChar(getMethodName(event), SPEC_CHAR_HYPHEN),
